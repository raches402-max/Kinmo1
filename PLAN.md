...
5. **W4 remaining** — Architecture cleanup: Slice 1 (group `server/` by domain) not started. Slice 3 (split `storage.ts`) **in progress** — 10 domains extracted so far (`reminders`, `frequency-feedback`, `user-profiles`, `category-search-history`, `time-slots`, `backups`, `group-collections`, `hosting`, `seen-activities`, `curated-venues`). storage.ts: 4,172 → 3,579 lines (~14% reduction). Pattern proven on a 217-line domain with an internal `this.` self-ref (backups). All ~660 callsites still unchanged. Remaining: member favorite venues, user/group saved places, standalone events, availability pulses, auto-scheduled events, scraped venues import — then the big ones (groups, members, itineraries, rsvps, activities, voting events).
