...
5. **W4 remaining** — Architecture cleanup: Slice 1 (group `server/` by domain) not started. Slice 2 (route splits) ✅ done. Slice 3 (split `storage.ts`) ✅ **complete** — 26 domains extracted, storage.ts 4,148 → **513 lines** (~88% reduction). Slice 4 Phase 1 ✅ shipped (`1994b9f` — 667 error sites swept to `fail()`); Phase 2 (success envelope wrap + frontend unwrap) still ahead.
...
**Slice 3: Split storage.ts** ✅ Complete (rev 28 — 88% reduction)
...
**Extracted (26 domains, storage.ts 4,148 → 513 lines):**
...
| member-group-preferences | 2 | upsert self-refs via module-local reference |
| voting-events | 11 | CRUD + group/global listings with aggregated upvotes/downvotes + votes table |
| activities | 9 | pure CRUD + feedback; no cross-refs |
| members | 9 | CRUD + 2 effective-value rollups (availability/budgets); inlines getGroup since groups domain not yet extracted |
| users | 3 | getUser, getUserByEmail, upsertUser (OIDC stable-id logic). Self-ref via module-local reference. `getUserGroups`/`getUserContacts` left in storage.ts — they fit with groups |
| itineraries | 13 | Biggest extraction yet (~485 lines): createItinerary (multi-source venue data + geocode), CRUD, items (add/update/reorder/delete), addAdHocVenueToItinerary, getSavedItineraries, getProposedItineraries (with time-slot vote aggregation via timeSlotsStorage). Cleaned up two inline `fetchItineraryWithItems` helpers in auto-scheduled-events and venue-visit-tracking that now use the real import |
| groups | 13 | The central hub (`ec0320d`): soft/hard delete cascade across voting-events + votes + backups, getUserGroups w/ orphan reconciliation + Tier-1 auto-link + standalone-invitee auto-link, getUserContacts, createAutomaticBackup (10-snapshot retention), cleanupOrphanedVotingData. Self-refs via groupsStorage.*; cross-domain ref to membersStorage.getGroupMembers (no cycle). `members.getGroupMembersBudgets` still inlines its own `groups` db.select to avoid a circular import — fine to leave |

**Slice 3 complete.** All meaningful domains extracted. storage.ts is now a 513-line shell: IStorage interface declarations + DatabaseStorage class as pure delegation. No further extraction targets justify their own session.

**Slice 4: Standardize response shape — Phase 1 ✅ shipped**

- **Phase 0 — error message leak-fix (shipped `7dfbe21`).** `server/lib/safe-error.ts` + 391 callsites swept. Production returns "Internal server error" instead of raw `error.message`; dev still returns real messages.
- **Phase 1 — error envelope shape (shipped `1994b9f` 2026-05-21).** `server/lib/responses.ts` exports `ok()` and `fail()` helpers. Swept **667 error sites** across `server/routes/` + `server/routes.ts` from `res.status(N).json({ message: X })` → `fail(res, N, X)` (envelope: `{ success: false, error: X }`). Frontend was already reading both `json.message` and `json.error` in `throwIfResNotOk`, so swap was transparent — zero frontend changes. Sweep mechanics + import-fix scripts kept at `scripts/sweep-error-responses.mjs` and `scripts/fix-fail-imports.mjs`.

Still remaining for this slice:
- **Phase 2 — success envelope.** Wrap `res.json(x)` → `ok(res, x)` (`{ success: true, data: x }`) and update `apiRequest`/`getQueryFn` in `client/src/lib/queryClient.ts` to auto-unwrap. ~260 bare `res.json(...)` callsites + ~40 already-wrapped + every frontend consumer that destructures the response. Higher blast radius — deserves its own session + testing window.
...
10. ✅ ~~Delete dead duplicate helpers in `server/routes.ts`~~ — verified 2026-05-20: `calculateNameSimilarity`, `getQualityThresholds`, `parsePriceLevel` are now *imported* (not redefined) into `routes.ts` and used live by `generateAndStoreActivities`. Nothing to delete.
11. ✅ ~~Refresh stale CLAUDE.md file-size claims~~ — shipped 2026-05-20. CLAUDE.md tree + Important Notes now reflect the post-Slice-2/Slice-3 reality (routes split into 31 modules, storage split into 26 modules).
...
13. ✅ ~~Delete 9 unused `Input` types in `server/validation-schemas.ts`~~ — verified 2026-05-20: no `Input`-suffixed type exports remain in that file. Already cleaned.
14. ✅ ~~Archive or delete `server/routes/SPLIT_PLAN.md`~~ — deleted 2026-05-20.
15. ✅ ~~Consolidate 6 duplicate prototype `<Route>` registrations in `client/src/App.tsx`~~ — verified 2026-05-20: prototype routes are registered once, outside both the auth and unauth `<Switch>` branches. No duplication.
16. ✅ ~~Triage 2 active TODO comments~~ — verified 2026-05-20: both TODOs (`server/auto-scheduler.ts:502`, `server/routes/standalone-events.ts:273`) are gone — `grep TODO` returns no hits in either file. Resolved by earlier work.
