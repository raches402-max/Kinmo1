...
5. **W4 remaining** — Architecture cleanup: Slice 1 (group `server/` by domain) not started. Slice 3 (split `storage.ts`) **started 2026-05-20** — pattern established (`server/storage/README.md`), `reminders` + `frequency-feedback` extracted as proof. 4,172 → 4,148 lines so far. Pattern is: domain file exports an object; `DatabaseStorage` class uses field assignment (`logReminder = remindersStorage.logReminder`) so all ~660 callsites stay unchanged. Future sessions grind through remaining ~19 domains following the pattern.
