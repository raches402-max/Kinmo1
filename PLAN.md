...
5. **W4 remaining** — Architecture cleanup: Slice 1 (group `server/` by domain) not started. Slice 3 (split `storage.ts`) ✅ **complete** — 26 domains extracted. storage.ts: 4,148 → **513 lines** (~88% reduction). All ~660 callsites unchanged via field-assignment delegation pattern. storage.ts is now a pure shell — IStorage interface + DatabaseStorage class of delegations. Slice 2 and 4 still open (route splits / response standardization).
