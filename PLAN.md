...
5. **W4 remaining** — Architecture cleanup: Slice 1 (group `server/` by domain) not started. Slice 3 (split `storage.ts`) **in progress** — 23 domains extracted (`reminders`, `frequency-feedback`, `user-profiles`, `category-search-history`, `time-slots`, `backups`, `group-collections`, `hosting`, `seen-activities`, `curated-venues`, `saved-places`, `availability`, `standalone-events`, `auto-scheduled-events`, `admin-stats`, `scraped-venues-import`, `rsvps`, `preference-signals`, `venue-visit-tracking`, `member-group-preferences`, `voting-events`, `activities`, `members`). storage.ts: 4,148 → **1,506 lines** (~64% reduction). All ~660 callsites still unchanged. Remaining: the biggest load-bearing domains — users (OIDC + member linking), members, groups (~600 lines incl. soft-delete + auth), activities, voting events + votes, itineraries (the biggest at ~600 lines), itinerary invites, member-group preferences, saved itineraries. These have heavier cross-method coupling (groups ↔ members ↔ itineraries ↔ rsvps ↔ activities) so each warrants a fresh session with full attention rather than a tail-end push.
...
**Slice 3: Split storage.ts** 🔄 In progress (rev 26 — 64% reduction so far)
...
**Extracted (23 domains, storage.ts 4,148 → 1,506 lines):**
...
|---|---|---|
| user-operations | ~290 | OIDC upsert + getUserGroups + getUserContacts; called from auth middleware |
| groups | ~600 | Soft-delete, auth checks, preference rollups; central hub. Note: `members` already inlines a direct `groups` db.select for budget defaults — that can switch back to a real import once groups is extracted. |
