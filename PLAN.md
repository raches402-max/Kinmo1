...
5. **W4 remaining** — Architecture cleanup: Slice 1 (group `server/` by domain) not started. Slice 3 (split `storage.ts`) **near done** — 25 domains extracted (the prior 23 plus `users`, `itineraries`). storage.ts: 4,148 → **961 lines** (~77% reduction). Only **groups** remains as a non-trivial extraction. All ~660 callsites still unchanged. Remaining: the biggest load-bearing domains — users (OIDC + member linking), members, groups (~600 lines incl. soft-delete + auth), activities, voting events + votes, itineraries (the biggest at ~600 lines), itinerary invites, member-group preferences, saved itineraries. These have heavier cross-method coupling (groups ↔ members ↔ itineraries ↔ rsvps ↔ activities) so each warrants a fresh session with full attention rather than a tail-end push.
...
**Slice 3: Split storage.ts** 🔄 In progress (rev 27 — 77% reduction so far)
...
**Extracted (25 domains, storage.ts 4,148 → 961 lines):**
...
|---|---|---|
| groups | ~600 | Soft-delete fanout (touches members/itineraries/rsvps/voting-events/activities/backups/hosting), auth checks, preference rollups, getUserGroups + getUserContacts, cleanupOrphanedVotingData, hardDeleteGroup, createAutomaticBackup. Central hub — handle with care. Note: `members.getGroupMembersBudgets` inlines a direct `groups` db.select for budget defaults; that can switch back to a real import once groups is extracted. |
