...
-   **Google Places API (New)**: Migrated from legacy API to new Places API (v1) using direct HTTP requests.
    ...
    -   **Auto-Caching System**: High-quality API results (100+ reviews, 4.0+ rating) are automatically saved to the curated venues table after each search, creating a "self-learning" system. Only SF venues are cached (geographic validation), and duplicates are prevented via Place ID checks. The cache grows organically over time, progressively reducing API calls for popular searches. Cached venues are marked with `source: 'api_auto'` for tracking.
