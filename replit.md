...
- **Data Loss Prevention & Auto-Reconciliation**: Critical fix to prevent group deletion during Replit auth session recreation. Changed `groups.userId` and `groupBackups.userId` foreign keys from `onDelete: "cascade"` to `onDelete: "set null"` (with nullable columns). When auth issues cause user record deletion/recreation, groups and backups are preserved with null userId instead of being deleted. Automatic reconciliation logic in `getUserGroups()` detects orphaned groups/members by matching email addresses and re-links them to the new user ID on first login. Handles both organizer groups and invited member groups. All backups continue running even for orphaned groups.
...
- **AI Suggestion Generation**: GPT-4o generates diverse venue suggestions across categories, with category-aware retry logic and optimized prompt structure. **Performance Optimizations (Oct 2025)**: Condensed AI prompt from 400+ to ~150 lines (60% shorter) while preserving all data/filters for faster GPT-4o processing. Eliminated redundant Google Places API calls - photo data now parsed directly from Text Search results (saves 15+ API calls per generation). Optimized deduplication query to fetch only venue name fields instead of full Activity objects. Removed unnecessary session cache clearing to preserve cache across retries. Includes category filtering, on-demand category-specific generation with custom location/radius, and a "Multi-venue outing" toggle for sorting preferences.
...
- **Admin Dashboard**: Platform health monitoring dashboard accessible at `/admin` for administrators, displaying key metrics, trends, and visualizations. Includes multiple tabs:
  - **Analytics Tab**: Interactive venue analytics dashboard showing curated venues distribution. Displays summary cards (total venues, avg rating, avg reviews, categories) and an interactive region×category pivot table. Cells are clickable to drill down into filtered venue lists with detailed information (name, address, rating, reviews, source). Sparse coverage alerts highlight cells with <10 venues in red. Powered by GET `/api/admin/venue-analytics` (summary & breakdown) and GET `/api/admin/venues-by-filter` (drill-down data).
  - **Maintenance Tab**: Database cleanup tools including:
...
-   **Google Places API (New)**: Migrated from legacy API to new Places API (v1) using direct HTTP requests.
    ...
    -   **Cache-First Strategy with Curated Venues**: Prioritizes pre-loaded San Francisco venue data over API calls for dramatic cost reduction (10-100x faster). Uses `curated_venues` table with strict geographic validation (SF bounding box + location string matching) to prevent venue leakage. On-demand photo fetching: first request fetches photos from Place Details API and caches in DB; subsequent requests use cached photos (zero API calls). For SF searches: returns 10-50ms (cache hit) vs 500-1500ms (API call). Curated venues include Place IDs, coordinates, ratings, addresses. Bulk import endpoint `/api/admin/import-venues` supports loading venues from external datasets. **Fuzzy Venue Matching (Oct 31, 2025)**: Fixed critical bug where curated venues were selected by category only, ignoring AI-suggested venue names, causing duplicate selections (e.g., "Crossfit Burlingame" selected for multiple different AI suggestions). Implemented `calculateNameSimilarity()` using Jaccard word overlap + length penalty to rank filtered venues by name similarity. Only accepts matches above 60% threshold, falls back to Google Places API for low-confidence matches. Both main generation and category regeneration endpoints now log matched venue name and similarity score for debugging. **API Fallback with Cache Bypass & Photo Flexibility (Oct 31, 2025)**: Fixed "only 1 result" regression where API fallback was triggered but results were rejected due to missing `photoUrl`. Root cause: Google Places Text Search doesn't always return photos (requires separate Place Details call). Solution: Made `photoUrl` optional in all 3 validation points (category regen, curated matching, API fallback) - only `rating` and `address` are required. System's existing on-demand photo fetching handles missing photos. When curated filtering fails (low similarity or filter rejection), calls `searchPlaces(skipCurated=true)` to bypass all caches and force fresh Google Places API lookup. **Category Filter Schema Bug (Oct 31, 2025)**: Fixed critical bug where category filter was checking camelCase properties (mealEnabled, cafeEnabled, etc.) but database returns snake_case (meal_enabled, cafe_enabled). All category filter checks now use correct snake_case property names matching the schema.
